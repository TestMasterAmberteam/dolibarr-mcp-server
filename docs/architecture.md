...
3. One `MCPServer` instance dispatches 18 allowlisted reporting and sales tools.
4. `DolibarrClient` owns one `httpx2.AsyncClient` pool, the fixed `/users/info` operation, and
   fixed project, third-party, contact-relation, user-label, and time-entry REST operations.
...
9. Reporting and sales tools attach the same key only to individual, fixed-path API requests.
   Dolibarr authorizes every read or write with the caller's actual permissions.
...
`PROJECTLEADER` relations. Additional Dolibarr fields are ignored during validation and cannot
reach tool results. Search and aggregate tools omit notes.
